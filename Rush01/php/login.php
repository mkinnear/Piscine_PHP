...
if (isset($_POST['signup']) && isset($_POST['login']) && isset($_POST['pass1']) && isset($_POST['pass2']))
{
	...
	if ($pass1 == $pass2)
	{
		$count = $db->prepare("SELECT login FROM users WHERE login = ?");
		...
		else 
		{
			...
			$count = $db->prepare("INSERT INTO users (login, pass) VALUES (?, ?)");
...
else if ($_POST['signin'] && isset($_POST['login']) && isset($_POST['passwd']))
{
	...
	$count = $db->prepare("SELECT id, login FROM users WHERE login = ? AND pass = ?");
	$count->execute(array($login, $hash));
	$boum = $count->fetch();
	if ($boum['login'] === $login)
	{
...
<body>
	<header>
		<img src="../img/eloko.png" alt="Warhammer 40000" title="Warhammer 40000" />
	</header>
	<div id="middle">
		<div id="login">
			<h1>Log In</h1>
			...
			<input type="submit" name="signin" value="Login" class="sub">
			</form>
		</div>
		<div id="signup">
			<h1>Sign up</h1>
			...
			<br>
			<input type="hidden" value="submit">
			<input type="submit" name="signup" value="Register" class="sub">
			</form>
		</div>
